...
class FEMEngine(nn.Module):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.domain_length = kwargs.get('domain_length', 2.0)
...
class ResidualLoss(FEM3D):
    ...
    def forward(self, dataX, dataY):
        ...
        gpw = self.gpw
        trnsfrm_jac = 0.5*0.5*0.5 #(0.5*self.hx)*(0.5*self.hy)*(0.5*self.hz)
        JxW = (gpw*trnsfrm_jac).unsqueeze(-1).unsqueeze(-1).unsqueeze(0).to(dataY.device)
        JxW = JxW.unsqueeze(-1)
        
        u_gp = self.gauss_pt_evaluation(u)
        v_gp = self.gauss_pt_evaluation(v)
        w_gp = self.gauss_pt_evaluation(w)
        ...
        v_elm_gp = v_gp * JxW
        w_elm_gp = w_gp * JxW
        u_x_elm_gp = u_x_gp * JxW
        u_y_elm_gp = u_y_gp * JxW
        u_z_elm_gp = u_z_gp * JxW
        v_x_elm_gp = v_x_gp * JxW   
        v_y_elm_gp = v_y_gp * JxW 
        v_z_elm_gp = v_z_gp * JxW
        w_x_elm_gp = w_x_gp * JxW
        w_y_elm_gp = w_y_gp * JxW        
        w_z_elm_gp = w_z_gp * JxW
        
        u_elm = torch.sum(u_elm_gp, 1)
        v_elm = torch.sum(v_elm_gp, 1)
        w_elm = torch.sum(w_elm_gp, 1)
        u_x_elm = torch.sum(u_x_elm_gp, 1)
        u_y_elm = torch.sum(u_y_elm_gp, 1)
        u_z_elm = torch.sum(u_z_elm_gp, 1)
        v_x_elm = torch.sum(v_x_elm_gp, 1)        
        v_y_elm = torch.sum(v_y_elm_gp, 1)
        v_z_elm = torch.sum(v_z_elm_gp, 1)
        w_x_elm = torch.sum(w_x_elm_gp, 1)
        w_y_elm = torch.sum(w_y_elm_gp, 1)        
        w_z_elm = torch.sum(w_z_elm_gp, 1)
        
        return u_elm, v_elm, w_elm, u_x_elm, u_y_elm, u_z_elm, v_x_elm, v_y_elm, v_z_elm, w_x_elm, w_y_elm, w_z_elm
